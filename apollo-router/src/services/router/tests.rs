...
async fn it_stores_operation_error_when_config_is_enabled() {
    ...
    let mut telemetry_config = Conf::default();
    telemetry_config
        .apollo
        .errors
        .experimental_otlp_error_metrics = OtlpErrorMetricsMode::Enabled;
    let mut config = Configuration::default();
    config.apollo_plugins.plugins.insert(
        "telemetry".to_string(),
        serde_json::to_value(telemetry_config).unwrap(),
    );
    ...
    assert_counter!(
        ...
        &[
            ...
            KeyValue::new("graphql.operation.type", query),
